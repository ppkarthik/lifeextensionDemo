...
;
!function(b){"function"==typeof define&&define.amd?define(["jquery"],b):b("object"==typeof exports?require("jquery"):window.jQuery||window.Zepto)
...
}aP.types=[],aL="",aP.ev=t.mainEl&&t.mainEl.length?t.mainEl.eq(0):aN,t.key?(aP.popupsCache[t.key]||(aP.popupsCache[t.key]={}),aP.currTemplate=aP.popupsCache[t.key]):aP.currTemplate={},aP.st=aQ.extend(!0,{},aQ.magnificPopup.defaults,t),aP.fixedContentPos="auto"===aP.st.fixedContentPos?!aP.probablyMobile:aP.st.fixedContentPos,aP.st.modal&&(aP.st.closeOnContentClick=!1,aP.st.closeOnBgClick=!1,aP.st.showCloseBtn=!1,aP.st.enableEscapeKey=!1),aP.bgOverlay||(aP.bgOverlay=at("bg").on("click"+aB,function(){aP.close()
...
ar("FirstMarkupParse",b),aP.currTemplate[e]=b?aQ(b):!0
...
if(b||2!==j.which&&!j.ctrlKey&&!j.metaKey){var a=void 0!==h.disableOn?h.disableOn:aQ.magnificPopup.defaults.disableOn;
...
g.data&&(h=aQ.extend(g.data,h)),ar(aF,[a,h,g]),aQ.each(h,function(b,j){if(void 0===j||j===!1){return !0
}if(f=b.split("_"),f.length>1){var i=a.find(aB+"-"+f[0]);
if(i.length>0){var e=f[1];
...
aQ.magnificPopup.registerModule("gallery",{options:{enabled:!1,arrowMarkup:'<button title="%title%" type="button" class="mfp-arrow mfp-arrow-%dir%"></button>',preload:[0,2],navigateByImgClick:!0,arrows:!0,tPrev:"Previous (Left arrow key)",tNext:"Next (Right arrow key)",tCounter:"%curr% of %total%"},proto:{initGallery:function(){var d=aP.st.gallery,b=".mfp-gallery",a=Boolean(aQ.fn.mfpFastClick);
return aP.direction=!0,d&&d.enabled?(aL+=" mfp-gallery",au(aE+b,function(){d.navigateByImgClick&&aP.wrap.on("click"+b,".mfp-img",function(){return aP.items.length>1?(aP.next(),!1):void 0
}),aN.on("keydown"+b,function(c){37===c.keyCode?aP.prev():39===c.keyCode&&aP.next()
})
}),au("UpdateStatus"+b,function(e,f){f.text&&(f.text=W(f.text,aP.currItem.index,aP.items.length))
}),au(aF+b,function(c,k,j,i){var h=aP.items.length;
j.counter=h>1?W(d.tCounter,i.index,h):""
}),au("BuildControls"+b,function(){if(aP.items.length>1&&d.arrows&&!aP.arrowLeft){var j=d.arrowMarkup,i=aP.arrowLeft=aQ(j.replace(/%title%/gi,d.tPrev).replace(/%dir%/gi,"left")).addClass(ay),g=aP.arrowRight=aQ(j.replace(/%title%/gi,d.tNext).replace(/%dir%/gi,"right")).addClass(ay),c=a?"mfpFastClick":"click";
i[c](function(){aP.prev()
}),g[c](function(){aP.next()
}),aP.isIE7&&(at("b",i[0],!1,!0),at("a",i[0],!1,!0),at("b",g[0],!1,!0),at("a",g[0],!1,!0)),aP.container.append(i.add(g))
}}),au(aD+b,function(){aP._preloadTimeout&&clearTimeout(aP._preloadTimeout),aP._preloadTimeout=setTimeout(function(){aP.preloadNearbyImages(),aP._preloadTimeout=null
},16)
}),void au(aJ+b,function(){aN.off(b),aP.wrap.off("click"+b),aP.arrowLeft&&a&&aP.arrowLeft.add(aP.arrowRight).destroyMfpFastClick(),aP.arrowRight=aP.arrowLeft=null
})):!1
...
}}}}),function(){var a=1000,j="ontouchstart" in window,i=function(){av.off("touchmove"+g+" touchend"+g)
},h="mfpFastClick",g="."+h;
aQ.fn.mfpFastClick=function(b){return aQ(this).each(function(){var q,p=aQ(this);
if(j){var o,f,e,d,c,r;
p.on("touchstart"+g,function(k){d=!1,r=1,c=k.originalEvent?k.originalEvent.touches[0]:k.touches[0],f=c.clientX,e=c.clientY,av.on("touchmove"+g,function(l){c=l.originalEvent?l.originalEvent.touches:l.touches,r=c.length,c=c[0],(Math.abs(c.clientX-f)>10||Math.abs(c.clientY-e)>10)&&(d=!0,i())
}).on("touchend"+g,function(l){i(),d||r>1||(q=!0,l.preventDefault(),clearTimeout(o),o=setTimeout(function(){q=!1
},a),b())
})
})
}p.on("click"+g,function(){q||b()
})
})
},aQ.fn.destroyMfpFastClick=function(){aQ(this).off("touchstart"+g+" click"+g),j&&av.off("touchmove"+g+" touchend"+g)
}
}(),ap()
});
var stjude=stjude||{};
stjude.dprforms=stjude.dprforms||{};
stjude.dprforms.magnificPopup=(function(a){return{init:function(b){a(b).magnificPopup({type:"inline",closeOnContentClick:false,removalDelay:300,mainClass:"my-mfp-zoom-in"})
},initWithCallback:function(b,c){a(b).magnificPopup({type:"inline",closeOnContentClick:false,removalDelay:300,mainClass:"my-mfp-zoom-in",callback:{open:c}})
}}
})(jQuery);